...
public class Jumon {
    ...
    private void setItem(Item[] items) {
        // Itemは1-2が0バイト目、3-4が11バイト目、 5-6が3バイト目、 7-8が8バイト目
        int[] indices = {0, 11, 3, 8};

        for (int i = 0; i < indices.length; i ++){
            data[indices[i]] = (items[i * 2 + 1].code() << 4) | items[i * 2].code();
        }
    }
    ...
    // ボスモンスター討伐フラグの展開
    private void setBeatBossMonster(
            boolean beatDragon,
            boolean beatGolem) {
        // 見張りドラゴン退治
        if(beatDragon) {
    ...
    private void setCryptoCode(int cryptoCode) {
    ...
    private void setMagicCode()	{
        for (int i = 0; i < 14; i ++) {
            for (int j = 0; j < 8; j ++ ) {
                if (0 != (data[i] & (0x80 >> j))) {
                    data[14] ^= magicCode[i * 8 + j] ;
                }
            }
        }
    }

    // 16バイトの情報を6bitで分割して20文字分のデータにする
    ...
    public String toString() {
        ...
        for(int code : encodedCodes) {
            stringBuffer.append(jumonMap.get(code));
        }
